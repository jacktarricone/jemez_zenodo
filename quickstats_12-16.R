...
values(feb_12_26_hh_cor)[values(feb_12_26_hh_cor) == 0] <-NA
...
# hh mean full scene
cor_hh_list <-list.files("./feb12-26/HH", pattern = "*HH_01.cor*", full.names = TRUE)
...
# vv mean full scene
cor_vv_list <-list.files("./feb12-26/VV", pattern = "*VV_01.cor*", full.names = TRUE)
...
unw_hh_list <-list.files('./feb12-26/HH', pattern = "*HH_01.unw*", full.names = TRUE)
unw_hh <-rast(unw_hh_list[1])
values(unw_hh)[values(unw_hh) == 0] <-NA
plot(unw_hh)
unw_hh_pixels <-as.numeric(global(unw_hh, fun="notNA"))
hh_perc_lost <-round(100-(unw_hh_pixels/p3_total_pixels)*100, digits = 1)

# vv full scene pixels lost
unw_vv_list <-list.files('./feb12-26/VV', pattern = "*VV_01.unw*", full.names = TRUE)
unw_vv <-rast(unw_vv_list[1])
values(unw_vv)[values(unw_vv) == 0] <-NA
plot(unw_vv)
unw_vv_pixels <-as.numeric(global(unw_vv, fun="notNA"))
vv_perc_lost <-round(100-(unw_vv_pixels/p3_total_pixels)*100, digits = 1)

# study area pixels
hh_cor_study_area <-crop(cor_hh, vg)
hh_cor_study_area
...
unw_hh_vg_perc_lost_vg <-round(100-(unw_hh_vg_pixels/study_area_pixels)*100, digits = 1)

# vv
unw_vv_vg <-crop(unw_vv, vg)
plot(unw_vv_vg)
unw_vv_vg_pixels <-as.numeric(global(unw_vv_vg, fun="notNA"))
unw_vv_vg_perc_lost_vg <-round(100-(unw_vv_vg_pixels/study_area_pixels)*100, digits = 1)
