...
depth_data <-read.csv("/Users/jacktarricone/ch1_jemez/climate_station_data/vg/vg_met_data_v2.csv")
...
del_thresh = 15 # 5mm
depth_data$delta_depth <-c(0, diff(as.numeric(depth_data$vg_snow_depth_cm)))
...
depth_data$sd_interp_v3_smooth30 <- rollmeanr(depth_data$sd_interp_v3, 30, fill = NA)
...
ggplot(depth_data) +
  geom_point(aes(x = date_time, y = sd_interp_v2), col = "red", size = .3) +
  geom_line(aes(x = date_time, y = sd_interp_v3_smooth30))

write.csv(depth_data, "/Users/jacktarricone/ch1_jemez/climate_station_data/vg/vg_snow_depth_qaqc_NEW.csv" )
